...
본회 회칙 제${data.meetingType === "확대운영위원회" ? 35 : 40}조에 따라, 제${data.count ?? "X"}차 ${data.meetingType} (${now.getMonth() + 1}월, ${isRegularMeetingText})를 아래와 같이 진행합니다.
...
Thank you.`;
  ...
  static defaultTemplate(data: MeetingDetail): TemplateType {
    ...
    return {
      title: `${now.getFullYear()}년 제${data.count ?? "X"}차 ${data.meetingType} (${now.getMonth() + 1}월, ${data.isRegular ? "정기회" : "임시회"})`,
